...
- **Настраиваемое имя сервера в `vpn://` ссылке** (`--server-name=ИМЯ`, вопрос при первой установке). Раньше поле было зашито как `AWG Server`: все серверы в приложении Amnezia выглядели одинаково, а ручная правка терялась при перегенерации конфигов. Теперь имя задаётся при установке, сохраняется в `awgsetup_cfg.init` (ключ `AWG_SERVER_NAME`) и переживает переустановки; старые конфиги без ключа продолжают получать `AWG Server`. Значение валидируется (до 64 символов, без кавычек и бэкслеша) и экранируется при вложении в JSON. При смене имени печатается подсказка перевыпустить `vpn://` ссылки через `manage regen` (D#180)
