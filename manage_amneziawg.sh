...
modify_client() {
    ...
    case "$param" in
        AllowedIPs|DNS)
            ...
            # modify пишет ровно то, что попросили - это его контракт, и через
            # него же снимают ::/0 те, кому нужен работающий IPv6. Но если
            # заданный список это полный туннель БЕЗ ::/0, клиент теряет
            # маршрут, который ему дают add и regen, и его IPv6 снова идёт мимо
...
#   i1_mode: random (случайный "<r N>" ЛИБО пакет осмысленной формы - см. ветку
#           random ниже), absent (I1 не должно быть), binary ("<r N><b 0xHEX>")
...
_diag_cps_guard() {
    ...
    if [[ -r "$SERVER_CONF_FILE" ]]; then
        ...
        # мы не можем. Наш генератор даёт максимум 128 байт (до сентября 2026 было
        # 256), документированные рецепты - столько же, поэтому килобайт заведомо означает правку руками, а
        ...
        case "$rc_i1" in
            ...
            random)
                if [[ -n "$i1" && "$i1" =~ ^\<r\ [0-9]+\>$ ]]; then
                    ...
                    # 🔴 Не ругаться на собственный дефолт. С сентября 2026
                    # установщик пишет I1 формы DNS-ответа, и профиль оператора,
                    # собранный на форме `<r N>`, эту форму подтверждает, но
                    # обратного не утверждает. Прежняя редакция сваливала любой
                    # структурный пакет в «нестандартный формат» и тем толкала
                    # пользователя обратно к случайным байтам - ровно к тому,
                    # что на МТС замерено как неработающее.
                    #
                    # Что считать структурой - решает awg_cps_is_shaped, и её
                    # границы объяснены там же. Здесь важно, почему проверка
                    # вынесена в общую библиотеку: первые две редакции этого
                    # условия писались прямо тут и обе оказались слишком
                    # широкими (сначала подстрока `<b 0x...>`, потом якорь,
                    # пропускавший `<r 99>` и непереносимый `<c>`). Одна
                    # проверка в одном месте - и её можно испытать напрямую.
                    _diag_line OK "I1 структурный ($i1)"; ok=$((ok+1))
                    echo "        Профиль $carrier подтверждён на форме <r N>; структурный пакет на нём отдельно не замерялся. На МТС (Москва) пакет такой формы прошёл там, где случайный не прошёл ни разу."
                elif [[ -z "$i1" ]]; then
...
list_clients() {
    ...
    while IFS= read -r name; do
        ...
        # Срок действия. Читаем в ОБОИХ режимах вывода: таблице нужна строка
        # остатка, а JSON отдаёт expires_at (issue #250) - без него внешний
        # потребитель не отличит клиента со сроком от бессрочного, не читая
        # файлы на сервере.
        #
        # Три состояния, и они РАЗНЫЕ. Метки нет - клиент бессрочный, оба ключа
        # null. Метка прочитана и разобрана - число. Метка ЕСТЬ, но значение из
        # неё получить не удалось - expires_at остаётся null, а expires_at_error
        # становится "unreadable". Разница между первым и третьим принципиальна:
        # голый null означал бы «бессрочный по замыслу», хотя срок кто-то задал.
        # Причина, по которой не удалось, уходит в log_warn (stderr и журнал),
        # потому что потребителю JSON нужен один вопрос - можно ли доверять
        # значению, - а разбираться с конкретным файлом будет администратор.
        #
        # Числом считается ТОЛЬКО каноническая десятичная запись не длиннее 10
        # знаков. Ведущий ноль отвергается не из педантизма: JSON такие числа
        # запрещает, и строгий разборщик у потребителя (Python, .NET, Go) уронил
        # бы ВЕСЬ документ из-за одного клиента. Длина ограничена по той же
        # причине - значение за пределами int64 отвергнут Go и .NET, а bash
        # вдобавок молча заворачивает такое число в арифметике.
        # 🔴 Предел именно 15, а не «сколько похоже на дату»: parse_duration
        # верхней границы не имеет, и --expires=100000d даёт законные 11 знаков.
        # Более тесный предел молча ломал бы такой срок - клиент жил бы вечно, а
        # выдача называла его метку нечитаемой. 15 знаков заведомо внутри int64 и
        # внутри точного диапазона чисел JavaScript (2^53).
        #
        # Оба ключа присутствуют ВСЕГДА, «неприменимо» выражается значением
        # null. Ключ, появляющийся только иногда, делает форму записи зависящей
        # от данных, и строгий потребитель со сверкой набора полей на этом
        # спотыкается - ровно так спотыкается наш собственный контрактный тест
        ...
        if [[ ! "$name" =~ ^[a-zA-Z0-9_-]+$ ]]; then
            # Имя приходит из #_Name серверного конфига и подставляется в путь.
            # check_expired_clients валидирует его перед тем же действием, и по
            # той же причине: файл читается от root, а '../' в имени увёл бы
            # чтение за пределы каталога меток.
            log_warn "Клиент '$name': имя не проходит проверку, срок действия не читаю."
            _jexp_err='"unreadable"'
            [[ "$JSON_OUTPUT" -ne 1 ]] && exp_str=" [expiry не читается]"
        elif [[ -e "$_exp_file" || -L "$_exp_file" ]]; then
            # Проверка существования отдельно от чтения обязательна: помощник
            # возвращает пустую строку и для отсутствующего файла, и для
            # пустого, и для каталога на месте файла. Без неё сломанная метка
            # молча выдавалась бы за «клиент бессрочный».
            exp_ts=$(get_client_expiry "$name" 2>/dev/null) || _exp_rc=$?
            if [[ "$_exp_rc" -ne 0 ]]; then
                # Файл есть, а прочитать не вышло: ошибка ввода-вывода, запрет
                # модуля безопасности, обрыв чтения. Код возврата тут
                # единственный источник правды - вывод в этом случае пуст и
                # неотличим от пустого файла.
                # ⚠️ Отказ по ПРАВАМ сюда не относится: скрипт работает от root,
                # а root проверку прав обходит (замерено на стенде: chmod 000 на
                # метке значение не скрыл). Ветка нужна для остальных случаев.
                log_warn "Клиент '$name': метка срока не прочитана (код $_exp_rc), файл $_exp_file."
                _jexp_err='"unreadable"'
                [[ "$JSON_OUTPUT" -ne 1 ]] && exp_str=" [expiry не читается]"
                ...
                log_warn "Клиент '$name': метка срока не разобрана, файл $_exp_file."
                _jexp_err='"unreadable"'
                [[ "$JSON_OUTPUT" -ne 1 ]] && exp_str=" [expiry не читается]"
            fi
        fi
        ...
        for _cname in "${ARGS[@]}"; do
            ...
            if generate_client "$_cname"; then
                ...
                if [[ -n "$EXPIRES_DURATION" ]]; then
                    ...
                    # Та же каноническая форма, что в list_clients и в проверке
                    # истечения: иначе три места по-разному решали бы, что такое
