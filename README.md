...
| | |
|---|---|
| **Для нового сервера** | Ubuntu 24.04, Debian 13 |
| **Тоже поддерживается** | Ubuntu 26.04 |
...
| Ubuntu 26.04 | ✅ можно брать | до 2031-05-29 | PPA `noble` fallback применяется автоматически с v5.13.0 |
...
| :-------- | :--------------------- | :--------------------------- | :-----------: |
| `add`     | `<имя> [имя2 ...] [--expires=ВРЕМЯ] [--allowed-ips=СПИСОК]` | Добавить клиента(ов) (опц. с истечением и индивид. маршрутами) | Нет (авто) |
| `remove`  | `<имя> [имя2 ...]`     | Удалить клиента(ов)          |  Нет (авто) |
| `list`    | `[-v] [--json]`        | Список клиентов (`-v` детали, `--json` машиночитаемый с `client_ipv6` и `expires_at`) |       Нет     |
...
<details>
  ...
  <b>О:</b> Любой VPS с Ubuntu 24.04 LTS / Ubuntu 26.04 / Debian 13 (25.10 и Debian 12 тоже поддержаны, но обычная поддержка у них кончилась - см. таблицу совместимости), root-доступом и от 512 МБ RAM (рекомендуется 1 ГБ). Беру хостинги с незаблокированными IP и неограниченным трафиком. См. <a href="#recomend-hosting">рекомендацию</a> ниже.
</details>
